...
'''
    This script is a sanity check for twisted Fields.
    The twist should not affect simple freespace propagation.
...
'''
canvas_size = (N,N)
pattern = Drawing(canvas_size, 1.01)
...
'''
twcl = Crystal.from_expansion(e1+e2, epsi=n1**2, epse=n2**2)
etw = twcl.expansion
twcl.add_layer_custom("Sref",     ExtendedLayer(etw, Layer.half_infinite(e1, "reflexion", n1**2),  e2.g_vectors, 1))
twcl.add_layer_custom("Sbuffer",  ExtendedLayer(etw, Layer.uniform(e1, n1**2, 0.4),              e2.g_vectors, 1))
...
twcl.add_layer_custom("Sbuffer2", ExtendedLayer(etw, Layer.uniform(e2, n2**2, 0.4),              e1.g_vectors, 0))
twcl.add_layer_custom("Strans",   ExtendedLayer(etw, Layer.half_infinite(e2, "transmission", n2**2), e1.g_vectors, 0))

'''
    Define the device and solve.
'''
#device = ["Sbuffer", "Scylup", "Sair","Scyldo", "Sbuffer2"]
device = ["Sbuffer", "Sbuffer2"]
...
x, y, z = coords(0, 12, 0.5, 0.5, -2, zmax, (256, 1, 64))
...
ax.matshow(Edisp, cmap="RdBu", vmin=-vmax, vmax=vmax, aspect=3, extent=[ 0, np.max(x), 0, np.max(z)])
