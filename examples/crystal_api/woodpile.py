...
def solve_rt_notwist(frequency):
    pw = (pwx, pwx)
    ...
    """
        Define the device and solve.
    """
    device = ['1','2','3','4']  # , "Scylup"
    cl.set_device(device, [False] * len(device))
    cl.set_source(1 / frequency, polarization[0], polarization[1], theta, phi)
    cl.solve()
...
if sys.argv[1] == "ct":
    ...
    im = ax1.matshow(R.real-T.real, origin="lower", vmin=0, vmax=1, aspect=60/6, extent=extent)
    plt.colorbar(im)
